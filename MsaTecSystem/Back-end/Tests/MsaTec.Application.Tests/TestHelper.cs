...
public static class TestHelper
{
    ...
    public static List<ClienteViewModelForList> LoadClienteViewModelListsData()
    {
        var clientes = new List<ClienteViewModelForList>();

        foreach (var clnt in GenerateClientes())
        {
            var clienteId = Guid.NewGuid();
            var cliente = new ClienteViewModelForList
            {
                Id = clienteId,
                Nome = clnt.Value.Name,
                Email = clnt.Value.Email,
                TelefonePrincipal = new TelefoneViewModelForList()
                {
                    Id = Guid.NewGuid(),
                    Numero = ExtractTelefoneNumber(clnt.Value.Telefone),
